...
dat <- bind_rows(cfdna, buffy) %>%
    unite("uid", c("gene", "patient"), remove=FALSE) %>%
    group_by(uid) %>%
    nest()
is_valid <- function(dat){
    !any(is.na(c(dat$y, dat$n)))
}
...
is_nan <- is.nan(bayes_factor(dat2))
dat3 <- dat2[!is_nan, ]
## Adith, please check these
dat.nan <- dat2[is_nan, ]
tumor.derived <- dat3[bayes_factor(dat3) >= 10, ]
tumor.derived$IS %>%
    do.call(bind_rows, .)
chip.derived <- dat3[bayes_factor(dat3) <= 0.1, ]
chip.derived$IS %>%
    do.call(bind_rows, .)
## we might want to look at these more closely
undecided <- dat3[bayes_factor(dat3) > 0.1 & bayes_factor(dat3) < 10, ]
```

# Figures

For manuscript, choose an example from `tumor.derived`, `chip.derived`, and `undecided`.  For marginal likelihood, we are approximating an integral with by summing over many theta' -- think about a visualization that shows the area that we are approximating.

```{r examplefig}
## Choose an example where the Bayes factor is not ridiculously large
tumor.example <- tumor.derived[37, ]
## Rerun, saving the monte carlo samples
results <- importance_sampler(tumor.example$data[[1]], param.list,
                              save_montecarlo=TRUE)
##
## marginal likelihood integrand
## -- the theta's have a very small range for CTCs because of our strong prior andno observed allele counts in plasma
## - the 'importance sample' is not very heavy tailed
x <- results$densities %>%
    mutate(integrand=lik * prior/g,
           theta=results$samples$g) %>%
    select(theta, integrand, dna_source)
x %>%
    arrange(theta) %>%
    ggplot(aes(theta, integrand)) +
    geom_line() +
    geom_area(fill="gray", stat="identity") +
    theme_bw(base_size=16) +
    theme(panel.grid=element_blank()) +
    facet_wrap(~dna_source, scales="free")

## Evaluating with a different prior
param.list2 <- param.list
## uniform prior
param.list2$ctc <- list(a=1, b=9)
results2 <- importance_sampler(tumor.example$data[[1]],
                               param.list2,
                               save_montecarlo=TRUE)
```
