...
With our strong prior that CTCs are rare, we view the plasma MAF of 4/1000 and buffy coat MAF of 1/1000 as very weak evidence that the mutation is tumor derived (log Bayes factor `round(stats$bayesfactor$bayesfactor, 2)`).  Had we instead observed no mutations in buffy coat (scenario B below), the log Bayes factor increases by 3 and we would conclude that there is strong evidence in support of a tumor-derived mutation.

```{r scenariob}
importance_sampler(dat$data[[2]], param.list, save_montecarlo=FALSE)
```

Note, that the log marginal likelihood of the CTC model is higher as observing no mutations in 1000 reads from buffy coat is more consistent with our prior; and the log marginal likelihood of the chip model decreases 1 as we would anticipate observing mutant reads in the buffy coat if the mutation was derived from CHIP.
...
```{r tumor_derived}
## Tumor-derived
ctcs <- wbc_somatic(dat$data[[1]], param.list)
ctdna <- plasma_somatic(dat$data[[1]], param.list)
logmarglik <- log(ctcs$marglik) + log(ctdna$marglik)
```
...
ml_labels <- function(x){
    labels <- x$bayesfactor %>%
        ...
        mutate(marginal_lik=round(marginal_lik, 2),
               label=paste0("log m(y)=", marginal_lik))
    labels
}
ml <- ml_labels(stats)
stats$samples %>%
...
ml.tumor <- round(sum(ml$marginal_lik[1:2]), 2)
ml.chip <- round(ml$marginal_lik[3], 2)
bf <- round(stats$bayesfactor$bayesfactor, 2)
...
bfs <- mls %>%
    group_by(ctc.prior) %>%
    summarize(ml.tumor=sum(marginal_lik[dna_source%in% c("CTC", "ctDNA")]),
              ml.chip=marginal_lik[dna_source=="WBC"]) %>%
    mutate(bf=round(ml.tumor-ml.chip, 2))
d <- IS.list %>%
    map_dfr(ml_data)
...
all <- dat2$IS %>%
    do.call(bind_rows, .)
tumor.derived <- dat2[bayes_factor(dat2) >= log(10), ]
tumor.derived$IS %>%
    do.call(bind_rows, .)
chip.derived <- dat2[bayes_factor(dat2) <= log(0.1), ]
chip.derived$IS %>%
    do.call(bind_rows, .)
## we might want to look at these more closely
undecided <- dat2[bayes_factor(dat2) > log(0.1) &
                  bayes_factor(dat2) < log(10), ]
```

```{r vanterve.results}
cairodir <- here::here("../../jmedina/cairo5_mutations/docs/table/")
d <- file.path(cairodir, "probability_somatic.Rmd")
vanterve <- read_csv(file.path(d, "WBC_cfDNA_analysis.csv"),
                     show_col_types=FALSE) %>%
    unite(uid, c(patient_id, aachange), sep="_", remove=FALSE)


invlogit <- function(xb) exp(xb)/(1+exp(xb))
all2  <-  all %>%
    mutate(prob=invlogit(bayesfactor),
           prob=ifelse(is.nan(prob) & bayesfactor > 0, 1, prob),
           prob=ifelse(is.nan(prob) & bayesfactor < 0, 0, prob)) %>%
    mutate(uid=dat2$uid)
mafs.plasma <- dat2$data %>%
    do.call(bind_rows, .) %>%
    filter(analyte=="plasma")
mafs.bc <- dat2$data %>%
    do.call(bind_rows, .) %>%
    filter(analyte=="buffy coat")
mafs <- left_join(mafs.plasma, mafs.bc,
                  by=c("gene", "patient", "position"),
                  suffix=c(".plasma", ".buffy_coat")) %>%
    mutate(uid=dat2$uid)
isdat <- left_join(all2, mafs, by="uid") %>%
    rename(uid2=uid) %>%
    unite(uid, c(patient, position), sep="_", remove=FALSE) %>%
    rename(prob.is=prob)
combined <- left_join(vanterve, isdat, by="uid")
combined %>%
    ggplot(aes(probability.somatic, prob.is)) +
    geom_point() +
    geom_abline(intercept=0, slope=1)
discord <- filter(combined, prob.is > 0.75, probability.somatic < 0.05)
filter(dat2, uid %in% discord$uid2)$data
figdat %>%
    ggplot(aes(y.buffy_coat/n.buffy_coat,
               y.plasma/n.plasma)) +
    scale_y_log10() +
    scale_x_log10()
```
...
fig.d <- d %>%
    ...
    theme(strip.text.y=element_text(angle=0))
fig.d
bfs <- round(bayes_factor(examples), 2)
