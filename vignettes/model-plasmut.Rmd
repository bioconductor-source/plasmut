...
theta_s: MAF of mutation in cfDNA

y_s: observed mutation frequency in cfDNA. n_s is total number of distinct reads at this position
...
\begin{aligned}
p(data | M_s) &= \int_\theta p(data, \theta_w, \theta_s | M_s) d\theta\\
              &= \int_\theta p(data | \theta_w, \theta_s, M_s) p(\theta_w, \theta_s | M_s)d\theta\\
              &= \int_\theta p(y_s, n_s | \theta_s)p(y_w, n_w | \theta_w) p(\theta_w | M_s) p(\theta_s | M_s)
\end{aligned}
...
Specification of priors $p(\theta_w | M_s)$ and $p(\theta_s | M_s)$:

\begin{aligned}
\theta_s | M_s &\sim \text{Beta}(1, 10)\\
...
1.  Obtain Monte Carlo samples of Z, $\omega$ and $\theta_s$ from their respective prior distributions
2.  Compute $\theta_w$
3.  Compute the likelihood of data for each $\theta_w$ and $\theta_s$ (the two likelihoods are each binomial)
4.  Multiply the likelihoods by the prior
5.  Approximate the integral by the mean



To obtain the hematopoietic marginal likelihood we integrate over the prior distribution for the mutation allele frequency (MAF) in WBCs, denoted by $\theta_w$.  Similarly, the marginal likelihood that the mutation is tumor-derived is obtained by integrating over a prior for $\theta_p$.  The default prior for $\theta_p$ is a truncated normal distribution centered around theta_w with variance of 10 truncated on the unit interval.
